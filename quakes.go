...
func getBreakDates(params *QueryParams) []string {
	sql := "select to_char(origintime, 'YYYY-MM') as yrmth, count(*) as count from wfs.quake_search_v1"
...
func getQuakesCsv(w http.ResponseWriter, r *http.Request) {
	...
	// send result response
	w.Header().Set("Content-Disposition", `attachment; filename="earthquakes.csv"`)
...
func getSqlQueryString(sqlPre string, params *QueryParams) string {
	...
	if params.region != "" {
		sql += getSqlAndOrWhere(condition)
		sql += fmt.Sprintf(" ST_Contains((select geom from wfs.region where regionname = '%s'), origin_geom)  ", params.region)
		condition = true
	} else {
		// change bbox format
